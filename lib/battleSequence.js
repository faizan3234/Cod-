// COD 81-Frame WebP Dual-Canvas Battlefield Engine (Omnitrix-style architecture)
// Optimized Dual-Canvas: Canvas 1 paints WebP frames strictly on scroll change; Canvas 2 animates embers at 60fps
...
export function setupBattleSequence() {
  ...
  async function startPreloading() {
    ...
    // 3. Remainder
    setTimeout(() => {
      for (let i = 19; i < FRAME_COUNT; i++) {
        loadFrame(i);
      }
    }, 900);
  }

  // Canvas 1: Renders the battlefield frame ONLY when needed (no idle churn)
  function renderFrame(index, force = false) {
    if (!force && index === renderedFrameIndex) return;
    ...
    // Softened cinematic vignette gradients for depth without burying text in blackness
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0, 'rgba(3,3,3,0.55)');

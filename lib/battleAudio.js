...
// Continuous background playback (irrespective of tab visibility or actions) + synchronized beat-driven haptics
...
export function setupBattleAudio() {
  ...
  if (audioElement) {
    ...
    audioElement.addEventListener('pause', () => {
      // If paused unexpectedly by browser (e.g. backgrounding or interruption), auto-recover if not user-muted
      if (isPlaying && !userMuted) {
        setTimeout(() => {
          if (isPlaying && !userMuted && audioElement.paused) {
            void audioElement.play().catch(() => {});
          }
        }, 300);
      }
  ...
  async function startAudio() {
    ...
    if (audioElement) {
      ...
      try {
        ...
        // Autoplay may be blocked pending gesture, or decode issue
      }
    }
  ...
  // Dismiss deploy gateway and trigger massive deployment rumble
  ...
  // UNSTOPPABLE GESTURE UNLOCK: Any touch, pointer, click, key, or scroll starts audio
  const ensureUnlocked = () => {
    if (!audioUnlocked && !userMuted && !navigator.webdriver) {
      void startAudio();
    }
  };
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && isPlaying && !userMuted) {
      if (audioElement && audioElement.paused) {
        void audioElement.play().catch(() => {});
      }
      cancelAnimationFrame(animFrameId);
  ...
  window.addEventListener('focus', () => {
    if (isPlaying && !userMuted && audioElement && audioElement.paused) {
      void audioElement.play().catch(() => {});
    }
  });
  ...
  // Watchdog timer: checks every 1000ms to guarantee Dhurandhar is never silenced
  ...
  // Attempt instant autoplay on load (supported in PWA and permissive browsers)
  if (!navigator.webdriver) {
